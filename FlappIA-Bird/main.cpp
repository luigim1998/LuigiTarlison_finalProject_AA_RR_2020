    ...
    for(int j=Step; j<POPULACAO_TAMANHO; j++)
    {
        ...
        for(int k=0; k<mutations; k++)
        {
            ...
            switch(tipo)
            {
                ...
                case 3:
                {
                    Vetor[j]->DNA[indice] = Vetor[j]->DNA[indice] + (getRandomValue()*TAXA_APRENDIZADO); /// Soma com um valora aleatório multiplicado por uma taxa de aprendizagem
                }
                case 4:
                {
                    int indice_dna_troca = rand()%Step;
                    for(int l = indice; l < Vetor[j]->TamanhoDNA; l++){
                        Vetor[j]->DNA[l] = Vetor[indice_dna_troca]->DNA[l];
                    } // Faz cruzamento com um dos cromossomos que foram selecionados
                }
            }
        }
    }
